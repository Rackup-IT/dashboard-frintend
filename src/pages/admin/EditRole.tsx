...
import { MODULE_PERMISSIONS, type Permission } from "@/lib/roleStore";
...
export default function EditRole() {
  ...
  useEffect(() => {
    if (roleId) {
      apiRequest("GET", `role/get/${roleId}`).then((data) => {
        if (data) {
          setRoleTitle(data.role.name);
  ...
  const handleUpdate = () => {
    // all permissions value to lowercase

    apiRequest("PUT", `role/update-permissions/${roleId}`, {
      permissions,
    }).then(() => {
          ...
          <div className="space-y-6">
            ...
            <div className="border rounded-lg overflow-hidden">
              <table className="w-full">
                ...
                <tbody>
                  {Object.entries(MODULE_PERMISSIONS).map(
                            ...
                            {availablePermissions.map((permission) => (
                              ...
                              >
                                <Checkbox
                                  ...
                                  onCheckedChange={() =>
                                    handlePermissionToggle(module, permission)
                                  }
